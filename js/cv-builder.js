...
function addSkill(value){const clean=String(value||'').trim();if(!clean||skills.some(skill=>skill.toLowerCase()===clean.toLowerCase()))return;skills.push(clean);renderSkills();updatePreview();}
function updatePreview(){if(previews.name)previews.name.textContent=fields.name?.value.trim()||'Your Name';if(previews.role)previews.role.textContent=fields.role?.value.trim()||'Your target role';const contact=[fields.email?.value.trim(),fields.phone?.value.trim(),fields.location?.value.trim()].filter(Boolean);if(previews.contact)previews.contact.textContent=contact.join(' · ')||'Your contact details';if(previews.profile)previews.profile.textContent=fields.summary?.value.trim()||'Your introduction will appear here.';if(previews.skills)previews.skills.innerHTML=skills.length?skills.map(skill=>`<span>${escapeHtml(skill)}</span>`).join(''):'<span>Your skills will appear here.</span>';if(previews.experience)previews.experience.innerHTML=experiences.length?experiences.map(item=>`<div class="preview-entry"><strong>${escapeHtml(item.title||'Role')}</strong><small>${escapeHtml([item.company,item.dates].filter(Boolean).join(' · '))}</small><p>${escapeHtml(item.description||'Experience details will appear here.')}</p></div>`).join(''):'<p>Your experience will appear here.</p>';if(previews.education)previews.education.innerHTML=education.length?education.map(item=>`<div class="preview-entry"><strong>${escapeHtml(item.qualification||'Qualification')}</strong><small>${escapeHtml([item.institution,item.dates].filter(Boolean).join(' · '))}</small><p>${escapeHtml(item.details||'')}</p></div>`).join(''):'<p>Your education will appear here.</p>';}
...
document.querySelector('#add-skill')?.addEventListener('click',()=>{const input=document.querySelector('#skill-input');addSkill(input?.value);if(input){input.value='';input.focus();}});
...
function renderStep(){steps.forEach(step=>step.classList.toggle('active',Number(step.dataset.step)===currentStep));if(progress)progress.style.width=`${(currentStep/Math.max(steps.length,1))*100}%`;if(stepLabel)stepLabel.textContent=`${currentStep} of ${steps.length}`;if(nextButton)nextButton.innerHTML=currentStep===steps.length?'Finish my CV <span class="btn-arrow">✓</span>':'Next <span class="btn-arrow">→</span>';window.scrollTo({top:0,behavior:'smooth'});}
nextButton?.addEventListener('click',()=>{if(currentStep<steps.length){currentStep+=1;renderStep();return;}updatePreview();nextButton.classList.add('complete');nextButton.innerHTML='CV ready <span class="btn-arrow">✓</span>';window.setTimeout(()=>{window.location.href='cv-analysis.html';},450);});
